...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 10:17:22 2020
...
reverse = input('start with normal(n) or reverse(r) rules?')
...
if reverse:
    L_tone_freq = 4000 #frequency of sample tone in left lick trials
    R_tone_freq = 1000 #frequency of sample tone in right lick trials
elif not reverse:
...
if ttl_experiment == 'y':
    ...
    if sum(rewarded_trials[-16:]) > 12: #if 12 correct in the last 16 trials
        rewarded_trials = [] #reset rewarded_trials
        reverse = not reverse #switch the rule
        tone_L.Delete() #delete previous tones
        tone_R.Delete()

        if reverse: #reassign tone frequencies
            L_tone_freq = 4000
            R_tone_freq = 1000
        elif not reverse:
            L_tone_freq = 1000
        ...
        tone_L = core.tones(L_tone_freq, sample_tone_length) #create new tones instances
        tone_R = core.tones(R_tone_freq, sample_tone_length)

        print('-------------------RULE SWITCH-------------------')
        print(f'reverse rule = {reverse}. Left: {L_tone_freq}Hz, Right: {R_tone_freq}')
